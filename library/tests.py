...
class BaseTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.admin = User.objects.create_superuser(email='admin@example.com', password='admin')

        # Авторы, жанры, книги
        cls.author1 = Author.objects.create(name='Лев Толстой', bio='Война и мир', birth_date='1828-09-09')
        cls.author2 = Author.objects.create(name='Фёдор Достоевский', bio='Преступление и наказание', birth_date='1821-11-11')
...
class BookViewSetTest(BaseTestCase):
    ...
    def test_filter_books_by_title(self):
        response = self.client.get('/book/', {'title__icontains': 'война'})  # было {'title': 'война'}
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class BorrowRecordViewSetTest(BaseTestCase):
    ...
    def test_borrow_already_borrowed_not_returned(self):
        ...
        response = self.client.post('/borrowrecord/borrow/', {'book_id': self.book1.id})
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data['error'], 'You already borrowed this book and have not returned it')

    def test_return_book_success(self):
        """Успешный возврат книги."""
        borrow = BorrowRecord.objects.create(book=self.book1, user=self.user, is_returned=False)
        initial_copies = self.book1.available_copies
    ...
    def test_return_book_already_returned(self):
        ...
        borrow = BorrowRecord.objects.create(book=self.book1, user=self.user, is_returned=True)
        response = self.client.post(f'/borrowrecord/{borrow.id}/return_book/')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_return_book_unauthorized_user(self):
        ...
        borrow = BorrowRecord.objects.create(book=self.book1, user=self.admin, is_returned=False)
